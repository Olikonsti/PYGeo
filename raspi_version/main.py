...
class GeoSettings(LabelFrame):
    def __init__(self, geocanvas, color):
        self.geocanvas = geocanvas
        LabelFrame.__init__(self)

        winx = geocanvas.plotter.winx
        winy = geocanvas.plotter.winy

        self.place(x=winx/6, y=winy/3)

        self.delete_button = Button(self, text=" X ", bg="red", command=self.destroy)
        self.delete_button.place(x=670, y=0)

        self.config(bg=color, width=700, height=500)
